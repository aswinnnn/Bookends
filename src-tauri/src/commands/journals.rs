...
pub async fn create_journal(
    ...
    // Create the journal
    let r = crate::services::journal::create_journal(&conn, title, content, rawcontent, tags)
        .map_err(|e| e.to_string());

    if let Ok(id) = r {
        println!("Journal created successfully:\n{id:?}");
        window
            .emit("db:journal-created", ())
            .map_err(|e| e.to_string())?;
        return Ok(id);
    } else {
        window
...
pub async fn update_journal(
    ...
    // Update the journal
    let r = crate::services::journal::update_journal(&conn, id, title, content, rawcontent, tags)
        .map_err(|e| e.to_string());

    if r.is_ok() {
        window
            .emit("db:journal-updated", ())
            .map_err(|e| e.to_string())?;
    } else {
        window
...
pub async fn get_journal(
    ...
    window: tauri::Window,
    id: String,
) -> Result<Option<models::Journal>, String> {
    let db_st = app.state::<Mutex<DatabaseManager>>();
    let db_manager = db_st.lock().await;
    let conn = db_manager.get_connection().map_err(|e| e.to_string())?;
...
pub async fn get_journals(
    app: tauri::AppHandle,
    window: tauri::Window,
) -> Result<Vec<models::Journal>, String> {
    let db_st = app.state::<Mutex<DatabaseManager>>();
    let db_manager = db_st.lock().await;
    let conn = db_manager.get_connection().map_err(|e| e.to_string())?;
